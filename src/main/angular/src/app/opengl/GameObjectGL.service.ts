...
export class GameObjectGLService {
    ...
    public transformGameObject(gameObjectModel: GameObjectModel, clientRectangle: ClientRect): Array<number> {

        console.log(gameObjectModel);
        const array = [];
        const locationTrait = gameObjectModel.traitMap.LocationTrait as LocationTrait;
        const renderTrait = gameObjectModel.traitMap.RenderTrait as RenderTrait;
        renderTrait.modelPointList.map(value => {
            return {
                x: value.x + locationTrait.point2D.x,
                y: value.y + locationTrait.point2D.y
            }
        })
